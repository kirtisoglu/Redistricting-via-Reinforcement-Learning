...
def create_state_vector(num_blocks, num_districts, partition):

    A = np.zeros((num_districts, num_blocks))

    for d in range(1, num_districts + 1):
        blocks = list(partition[str(d)])
        for b in range(len(blocks)):
            A[d-1, blocks[b]] = 1
    return A.flatten()


def create_action_vector(num_blocks, num_districts, action):

    A = np.zeros((num_districts, num_blocks))

    b = list(action.items())[0][0]
    d = int(list(action.items())[0][1])
    A[d-1, b] = 1
    return A.flatten()

def group_keys_by_value(data):
    """
    Groups keys from a list of single-key dictionaries by their common values.

    Parameters:
        data (list of dict): A sorted list of dictionaries, each with a single key-value pair.

    Returns:
        dict: A dictionary where each key is a unique value from the input,
              and each value is a list of keys sharing that value.
    """
